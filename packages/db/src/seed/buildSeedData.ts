...
export function buildSeedData(now: Date = SEED_EPOCH, seed = 20260823) {
  ...
  // ---------------------------------------------------------------------------
  // Hardening E2E fixtures (deterministic ids used by tests/e2e):
  // Property prp_901 has TWO open maintenance cases (dishwasher / hot water)
  // so the Case Matcher must pick the right one from content evidence alone.
  // ---------------------------------------------------------------------------
  const fixtureContact = {
    ...
    notes: 'Hardening E2E fixture tenant',
    createdAt: now,
    updatedAt: now,
  };
  ...
  const fixtureLinks = [
    {
      ...
      validFrom: new Date(now.getTime() - 365 * 86400000),
      validTo: null,
      createdAt: now,
    },
  ];
  ...
  // --- PropertyMe-mirror sample entities (Spec §18–§21) -----------------------
  const week = 7 * 86400000;
  const fixtureTenancies = [
    {
      id: 'ten_901',
      agencyId: agency.id,
      propertyId: 'prp_901',
      tenantContactId: 'con_901',
      ...
      bondAmount: '2480.00',
      source: 'PROPERTYME' as const,
      externalId: 'PM-TEN-9001',
      sourceStatus: 'ACTIVE',
  ...
  const fixtureLeases = [
    {
      ...
      agencyId: agency.id,
      propertyId: 'prp_901',
      primaryTenantContactId: 'con_901',
  ...
  const fixtureMaintenanceJobs = [
    {
      ...
      priority: 'NORMAL' as const,
      tradeName: 'Brighton Appliance Repairs',
      quoteAmount: '190.00',
      loggedAt: new Date(now.getTime() - 10 * 86400000),
      completedAt: null,
      source: 'PROPERTYME' as const,
      externalId: 'PM-100001',
      sourceStatus: 'ACTIVE',
      syncStatus: 'SYNCED' as const,
      lastSyncedAt: now,
      sourceUpdatedAt: now,
  ...
  const fixtureInspections = [
    {
      ...
      status: 'SCHEDULED' as const,
      scheduledAt: new Date(now.getTime() + 3 * week),
      completedAt: null,
      notes: 'Routine quarterly inspection',
      source: 'PROPERTYME' as const,
      externalId: 'PM-INS-9001',
      sourceStatus: 'ACTIVE',
      syncStatus: 'SYNCED' as const,
      lastSyncedAt: now,
      sourceUpdatedAt: now,
  ...
  const fixtureMappings = [
    {
      id: 'exm_901',
      source: 'PROPERTYME' as const,
      sourceAccountId: 'bayside-pm',
      externalEntityType: 'MAINTENANCE_JOB',
      externalId: 'PM-100001',
      localEntityType: 'maintenance_job',
      localEntityId: 'mnt_901',
      sourceUpdatedAt: now,
      lastSyncedAt: now,
      sourceHash: null,
      syncStatus: 'SYNCED' as const,
  ...
  return {
    agencies: [agency],
    users: usersList,
    contacts: [...contactRows, fixtureContact],
    properties: [...propertyRows, fixtureProperty],
    propertyContacts: [...propertyContactRows, ...fixtureLinks],
    cases: [...caseRows, ...fixtureCases],
    ...
    tenancies: fixtureTenancies,
    leases: fixtureLeases,
    maintenanceJobs: fixtureMaintenanceJobs,
    inspections: fixtureInspections,
    externalEntityMappings: fixtureMappings,
  };
}
